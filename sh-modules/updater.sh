...
updater_download () {
    ...
    __result="$(wget -nv -O download.tar.gz -o /dev/stderr "https://github.com/${__repository}/archive/${__version}.tar.gz" 2>&1 )"
    if [ $? -ne "${CR_TRUE}" ]
    then
        console_error "${__result}"
    else 
        __result="$(tar -xzvf download.tar.gz 2>&1)"
        if [ $? -ne "${CR_TRUE}" ]; then
            console_error "${__result}"
        else
    ...
    # __result="$(cd "${__to_dir}" && wget -qOlog "https://github.com/${__repository}/archive/${__version}.tar.gz" | tar -xzv 2>&1)"
